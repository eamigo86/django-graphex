...
class FieldSpec:
    ...
    "name" is the snake_case source name; camelCase is derived at compile
...
class TypeSpec:
    ...
    Groups the type name with its ordered field specs and any GraphQL-only
    metadata the compiler reads at schema-build time.
    """
...
class EnumSpec:
    ...
    "values" is a sequence of "(graphql_name, raw_value)" pairs. The raw value
    is delivered via "GraphQLEnumValue(value=raw)" so callers receive the raw
    Python value, not a wrapped enum member.
    """
