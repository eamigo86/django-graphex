...
"DjangoUnionType" / "DjangoInterfaceType" as a plain "ObjectType" (both are now
"native.base.ObjectType" subclasses, S6d), so a union field compiled to a
...
- "DjangoUnionType" derived from graphene "Union" -> graphene built a
  ...
  to its registered "DjangoObjectType" (via "registry.get_type_for_model").
...
- "compile_interface_type" -> memoized "GraphQLInterfaceType" with the declared
  shared fields (compiled via the SAME declared-field compiler the object types
  use) + the bridged "resolve_type".
...
def is_union_type(cls: Any) -> bool:
    """Return whether the class is a "DjangoUnionType" subclass.

    Args:
        cls: The candidate object to classify.

    Returns:
        True when "cls" is a class and a "DjangoUnionType" subclass.
    """
    import inspect
...
def is_interface_type(cls: Any) -> bool:
    """Return whether the class is a "DjangoInterfaceType" subclass.

    Args:
        cls: The candidate object to classify.

    Returns:
        True when "cls" is a class and a "DjangoInterfaceType" subclass.
    """
    import inspect
...
def compile_union_type(
    ...
    Args:
        ...
        registries: The "SchemaRegistries" pair owning the union cache namespace;
...
def compile_interface_type(
    ...
    gql_interface = GraphQLInterfaceType(
        ...
        extensions={"gdx": GdxPayload(GdxMeta(name=name, graphene_type=interface_cls))},
    )
