r"""extensions["gdx"] bridge: GdxPayload, _MetaView, assert_gdx_bridge.
...
class _MetaView:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        try:
            ...
            raise AttributeError(f"_MetaView: GdxMeta has no attr {name!r}.")
...
class GdxPayload:
    ...
    The "_meta" property provides the "graphene_type._meta" shim so existing
    ...
    def __init__(self, gdx_meta: Any) -> None:
        """Store the wrapped "GdxMeta" under the "_gdx_meta" slot.

        Args:
            gdx_meta: The "GdxMeta" carrier this payload wraps and exposes
                through the "_meta" view.
        """
...
_BUILTIN_SCALAR_NAMES: frozenset[str] = frozenset(
    {
        t.name
        for t in (GraphQLString, GraphQLInt, GraphQLFloat, GraphQLBoolean, GraphQLID)
    }
)


def assert_gdx_bridge(schema: GraphQLSchema) -> None:
    """Assert every user-defined type in "schema" carries "extensions['gdx']".

    Covers object/interface/input types. Skips:
    ...
    Args:
        schema: The built "GraphQLSchema" whose type map is checked.

    Raises:
        AssertionError: Identifying the first object/interface/input type that
            is missing "extensions['gdx']".
    """
