...
This is the consumer half of the serialize-once data path (design §3). The
PRODUCER (``SubscriptionBinding.broadcast`` in ``bindings.py``) serializes a
...
class ChannelLayerSource:
    ...
    ----------
    groups:
        The EXACT Channels group names this source must join. The caller derives
        them from the requested action (``('create','update','delete')`` for
        ``all_actions`` else ``(action,)``) — this class never hardcodes all
        three (the #1420 guard).
    channel_layer:
        The Channels channel layer (``InMemoryChannelLayer`` in tests, Redis in
        production) the source receives from.
    filters:
        Optional notification filters (Django ORM lookup -> expected value)
        applied PRE-execute via :func:`split_filters`. Plain-equality keys present
        in the flat ``data`` are compared in memory; a mismatch DROPS the event
        (yields nothing). Defaults to an empty mapping (yield every event).

    Notes:
        The receive loop yields ``payload['data']`` — the already-serialized flat
        snake dict — verbatim. It performs NO re-serialization and NO model
        instantiation (the serialize-once invariant). ``__lookup`` filters
        returned by :func:`split_filters` as a NON-EMPTY "remaining" mapping
        require a single-row DB verification; that DB-backed narrowing is owned by
        the driver layer (WU5) which wires the :attr:`db_verify` hook to run an
        ``.exists()`` check. Until a hook is wired the source DROPS such events
        CONSERVATIVELY (never yields unverified — see the SECURITY note on the
        receive loop). The equality-only common case never issues a query.

    Attributes:
        db_verify:
            Optional async DB-verify hook ``async (remaining, event) -> bool``
            the driver (WU5) wires to verify a NON-EMPTY ``__lookup`` "remaining"
            filter mapping against the database (a single-row ``.exists()``
            narrowing). When ``None`` (the default), a non-empty remaining mapping
            DROPS the event conservatively — the source NEVER silently yields an
            unverified ``__lookup``-filtered event.
    """
    ...
    def channel(self) -> str | None:
        """The per-consumer channel name allocated at :meth:`start`."""
        return self._channel

    @property
    def joined_groups(self) -> list[str]:
        """The groups actually joined (a copy — callers cannot mutate state)."""
        return list(self._joined)

    @property
    def is_closed(self) -> bool:
        """Whether the source has been closed (groups discarded)."""
        return self._closed
    ...
    async def __anext__(self) -> dict[str, Any]:
        ...
        (unverified ``__lookup`` filters) is verified via :attr:`db_verify` when
