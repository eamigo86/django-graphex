...
(``native/bridge.py``).  For each subscription event (output) type:

1. **Type-level escape hatch**: if the type carries
   ``_gdx_pure_projection = True`` → bypass the guard entirely.  Advanced
   users may set this to accept the per-subscriber live-resolver cost.

2. **Field-level sentinel check**: for every field of the output type assert
   that ``field.resolve`` carries the sentinel::
   ...
   HARD-FAIL (``GraphQLError``) on ANY field whose resolve:
     - is ``None`` (no resolver set — would fall through to
       ``default_field_resolver`` which uses camelCase keys → silent NULL)
     - is ``default_field_resolver`` (same silent-null problem)
     - is a hand-written / live / computed / FK / permission resolver that
       lacks the sentinel

**CRITICAL (design §5)**: the check is the SENTINEL PRESENCE, NOT identity
...
def check_subscription_output_type(gql_type: GraphQLObjectType) -> None:
    ...
    ----------
    gql_type:
        The GraphQL output (event) type produced by a subscription.

    Raises:
    ------
    GraphQLError:
        If any field's resolver is missing the sentinel, is ``None``, or is the
        graphql-core ``default_field_resolver``.

    Notes:
    -----
    Type-level bypass: if ``gql_type._gdx_pure_projection is True`` the entire
    type is whitelisted — no field check is performed.  Use this escape hatch
    only when you intentionally accept live-resolver cost per subscriber.
    """
...
def check_subscription_schema(schema: GraphQLSchema) -> None:
    """Walk all subscription output types in *schema* and call check_subscription_output_type on each.

    This is the entry point for the build-time guard.  Call it in the same
    pass as ``assert_gdx_bridge`` after schema assembly.

    Parameters
    ----------
    schema:
        The fully-assembled ``GraphQLSchema``.

    Raises:
    ------
    GraphQLError:
        From :func:`check_subscription_output_type` on the first offending type.
    """
