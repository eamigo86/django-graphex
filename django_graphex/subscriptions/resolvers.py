...
the camelCase WIRE name (e.g. ``isActive``).  The serialized subscription
payload produced by ``native/backend.py:to_representation`` uses SNAKE keys
(e.g. ``is_active``).  This mismatch produces a SILENT NULL for every
multi-word field — the blocking correctness issue documented in design §6.

``make_snake_resolver`` closes over the snake-case key and reads it directly
from the source dict, bypassing the camelCase wire name entirely.
...
def make_snake_resolver(snake: str) -> Callable[..., Any]:
    ...
    ----------
    snake:
        The snake_case field name as it appears in the serialized subscription
        payload (e.g. ``"is_active"``, ``"date_joined"``).

    Returns
    -------
    Callable
        A two-argument resolver ``(root, info) -> value`` compatible with the
        graphql-core field resolver protocol.  The closure:

        - Reads ``root[snake]`` when *root* is a ``dict`` (the common
          production path — the flat serialized dict from
          ``native/backend.py:to_representation`` never touches the ORM).
        - Falls back to ``getattr(root, snake, None)`` for non-dict roots
          (direct object access in tests or non-serialized usage).
    """
