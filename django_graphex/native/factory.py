...
``native_factory_type(op, base, **kw)`` — parallel to the UNTOUCHED
``base_types.factory_type``.  Builds Django output/list types for the native
...
- ``"input"``:  delegates to Phase 2 input factory (not implemented in WU-A;
...
def native_factory_type(op: str, base: type, *args: Any, **kwargs: Any) -> type:
    ...
    Args:
        ...
        **kwargs: type configuration such as ``model``, ``name``,
                  ``only_fields``, ``exclude_fields``, ``results_field_name``,
                  ``pagination``, ``queryset``, ``registry``.

    Returns:
        The generated type class (a subclass of ``base``).

    Raises:
        NotImplementedError: for ``op="input"`` (delegated to Phase 2;
            call ``base_types.factory_type("input", ...)`` instead).
