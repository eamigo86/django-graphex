...
DESIGN RECONCILIATION (#1432 §3 vs §8): §8's "event type SDL byte-identical to
the DjangoObjectType OUTPUT type" goal is a CATEGORY ERROR vs §3's flat-pk
serialize-once payload — the output type's FK/M2M are DB-backed nested types, so
a serialize-once event (one serialize feeds N subscribers with ZERO DB per
subscriber) CANNOT deliver them without a per-subscriber DB query (the N+1 cliff
serialize-once forbids). The DELIVERABLE parity reference is what
``to_representation`` carries: FK -> pk scalar, M2M -> pk-list.
...
def test_event_type_fk_is_deliverable_pk_scalar_not_nested_object():
    ...
    DESIGN RECONCILIATION (#1432 §3): the serialize-once payload
    ...
    an auto/id pk), NOT the DB-backed nested ``AuthorT`` output type (which would
    need a per-subscriber DB query — the N+1 cliff serialize-once forbids). It is
    also NEVER the old ``Int`` seam.
    """
    _register_post_node_types()
    sub = _make_subscription()
    event_type = sub._build_native_event_type()
...
def test_event_type_m2m_is_deliverable_pk_list_not_container_not_string():
    ...
    renders it as ``[<pk scalar>]`` (auto pk -> ``[ID]``), NOT the DB-backed
    ``<Model>ListType`` results/totalCount container (whose ``totalCount`` resolver
    over a python list raises a coercion error), and NOT omitted, and NOT a bare
    ``String`` stand-in. The M2M is PRESENT and deliverable.
    """
    _register_post_node_types()
    sub = _make_subscription()
    event_type = sub._build_native_event_type()
...
def test_event_type_sdl_is_the_deliverable_flat_pk_shape():
    ...
    DESIGN RECONCILIATION (#1432 §3 vs §8): §8's "event type SDL byte-identical to
    the DjangoObjectType OUTPUT type" goal is a CATEGORY ERROR vs §3's flat-pk
    payload — the output type's FK/M2M are DB-backed nested types, which a
    serialize-once event CANNOT deliver without a per-subscriber DB query. The
    correct parity reference is what ``backend.to_representation`` actually carries:

      * scalars -> the Phase-5 native names/nullability (shared with the output
        type's scalar rendering),
      * FK / O2O -> the pk scalar (``ID``),
      * M2M -> a list of pk scalars (``[ID]``).
    """
...
async def test_serialize_once_delivers_over_full_converter_event_type():
    ...
    A scalar-only selection projects the serialize-once flat snake dict with NO
    coercion error (the FK pk + M2M list payload keys are simply not selected).
    """
    ...
    query_type = GraphQLObjectType("Query", lambda: {"ok": GraphQLField(GraphQLBoolean)})
...
async def test_nested_fk_and_m2m_selection_delivers_pks_zero_queries():
    ...
    DB queries — the snake-closure projects the flat dict, never the ORM. Against
    the old nested-object/container shapes this returned a fully-null FK object
    (silent wrong) and raised a totalCount coercion ``GraphQLError``.
    """
    ...
    doc = parse(
        "subscription { postEvent { id author category tags coAuthors } }"
    )
...
async def test_nested_m2m_totalcount_selection_is_rejected_at_validation():
    ...
    doc = parse(
        "subscription { postEvent { tags { totalCount results { id } } } }"
    )
