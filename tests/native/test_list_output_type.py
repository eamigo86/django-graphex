...
@pytest.mark.django_db
def test_list_output_type_has_gdx_extensions():
    """extensions['gdx'] must be set on the compiled list container type and
    must carry results_field_name (the pagination arg metadata required by WU5+)."""
    from django_graphex.native.registry_compiler import compile_all_outputs
    from django_graphex.types import DjangoListObjectType
    from tests.models import Category
...
@pytest.mark.django_db
def test_list_output_type_has_results_and_totalcount_fields():
    """The compiled container GraphQLObjectType must have 'results' (or the
    custom results_field_name) and 'totalCount' fields."""
    from django_graphex.native.registry_compiler import compile_all_outputs
    from django_graphex.types import DjangoListObjectType
...
@pytest.mark.django_db
def test_list_output_type_results_element_is_canonical_node():
    """The results field's element type MUST be the single canonical
    GraphQLObjectType for the node (same instance as node._meta.graphql_output_type),
    NOT a GraphQLString / duplicate / second instance."""
...
@pytest.mark.django_db
def test_list_output_type_schema_assembly_no_duplicate():
    """Schema assembly smoke test: list container type + its node + a mutation
    referencing the node → GraphQLSchema constructs WITHOUT duplicate-name TypeError.
