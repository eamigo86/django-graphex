...
Run: .venv/bin/python -m pytest \
    tests/native/test_registry_compiler_coverage.py -q -o addopts=""
"""
...
def test_compile_one_returns_already_compiled_type() -> None:
    """``_compile_one`` returns an existing compiled type without rebuilding it."""
...
def test_compile_one_returns_in_progress_stub_during_cycle() -> None:
    """During a cycle, ``_compile_one`` returns the in-progress stub for a model."""
    from django_graphex.native import registry_compiler
    from django_graphex.native.registry_compiler import (
        NativeOutputRegistry,
        _compile_one,
        _reset_in_progress,
...
def test_unregistered_related_model_is_dropped_with_warning(caplog) -> None:
    """A related model never compiled / in-progress is dropped, with a warning.

    The relation field must NOT appear in the compiled type, and the warning must
    name BOTH the dropped field and the unregistered target (rank-6 contract).
    """
    from django_graphex.native.registry_compiler import (
        NativeOutputRegistry,
        compile_all,
    ...
    with caplog.at_level(
        logging.WARNING, logger="django_graphex.native.registry_compiler"
    ):
...
def test_registered_related_model_resolves_to_its_compiled_type() -> None:
    """A related model that IS registered resolves to its compiled instance.
...
def test_compile_all_invokes_model_rebuild_when_present() -> None:
    """A class exposing ``model_rebuild`` has it called during Phase 1."""
    from django_graphex.native.registry_compiler import (
        NativeOutputRegistry,
        compile_all,
    )
...
def test_compile_all_swallows_model_rebuild_exception() -> None:
    """A ``model_rebuild`` that raises is swallowed; the type still compiles."""
    from django_graphex.native.registry_compiler import (
        NativeOutputRegistry,
        compile_all,
    )
...
def test_compile_all_outputs_returns_early_when_registry_empty(monkeypatch) -> None:
    """With an EMPTY global output registry, ``compile_all_outputs`` is a no-op.

    Patches the global ``_gdx_output_registry`` to empty so the early-return arm
    runs without touching the real app-ready state.
    """
    from django_graphex.native import base, registry_compiler
