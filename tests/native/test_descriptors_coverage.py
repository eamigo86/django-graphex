...
Run: .venv/bin/python -m pytest \
    tests/native/test_descriptors_coverage.py -q -o addopts=""
"""
...
def test_source_resolver_reads_dict_key() -> None:
    """``_source_resolver`` reads the key off a mapping root."""
    from django_graphex.native.descriptors import _source_resolver
...
def test_source_resolver_reads_missing_dict_key_as_none() -> None:
    """A missing dict key resolves to ``None`` (``dict.get`` default)."""
    from django_graphex.native.descriptors import _source_resolver
...
def test_source_resolver_reads_attribute_off_object() -> None:
    """``_source_resolver`` reads the attribute off a non-mapping root."""
    from django_graphex.native.descriptors import _source_resolver
...
def test_source_resolver_calls_method_result() -> None:
    """A source pointing at a zero-arg method is CALLED (graphene parity)."""
    from django_graphex.native.descriptors import _source_resolver
...
def test_source_resolver_calls_function_attribute() -> None:
    """A source pointing at a plain function attribute is CALLED."""
    from django_graphex.native.descriptors import _source_resolver
...
def test_resolve_thunk_imports_dotted_string() -> None:
    """A dotted import-path string is resolved to the imported object."""
    from django_graphex.native.descriptors import _resolve_thunk
...
def test_resolve_thunk_calls_zero_arg_lambda() -> None:
    """A zero-arg callable (lazy forward ref) is invoked and its result returned."""
    from django_graphex.native.descriptors import _resolve_thunk
...
def test_resolve_thunk_calls_partial() -> None:
    """A ``functools.partial`` is invoked (graphene ``get_type`` parity)."""
    from django_graphex.native.descriptors import _resolve_thunk
...
def test_resolve_thunk_returns_non_thunk_verbatim() -> None:
    """A concrete type (graphql-core scalar) is returned unchanged."""
...
def test_next_creation_counter_is_strictly_monotonic() -> None:
    """Successive counters increase by exactly 1 (declaration-order parity)."""
    from django_graphex.native.descriptors import _next_creation_counter
...
def test_mounted_field_required_wraps_type_in_native_nonnull() -> None:
    """``required=True`` wraps the declared type in a ``NativeNonNull``."""
    from graphql import GraphQLString

    from django_graphex.native.descriptors import NativeMountedField, NativeNonNull
...
def test_mounted_field_source_becomes_resolver() -> None:
    """A ``source=`` (with no explicit resolver) installs a source resolver."""
    from graphql import GraphQLString
...
def test_mounted_field_explicit_resolver_wins_over_source() -> None:
    """An explicit resolver is NOT overridden by a ``source`` kwarg."""
    from graphql import GraphQLString
...
def test_mounted_field_wrap_subscribe_passthrough() -> None:
    """``wrap_subscribe`` returns the parent subscribe unchanged (graphene parity)."""
    from graphql import GraphQLString
...
def test_mounted_field_ordering_dunders() -> None:
    """``__eq__`` / ``__lt__`` / ``__hash__`` order by ``creation_counter``.
...
def test_mounted_field_eq_with_non_descriptor_is_not_implemented() -> None:
    """Comparing to a non-descriptor falls back to Python's default (not equal)."""
    from graphql import GraphQLString
...
def test_mounted_field_lt_with_no_counter_object_raises_typeerror() -> None:
    """``__lt__`` against an object lacking ``creation_counter`` is unorderable."""
    from graphql import GraphQLString
...
def test_relation_field_is_inert_marker_with_counter() -> None:
    """``NativeRelationField`` carries a counter and an inert ``None`` type."""
...
def test_relation_field_allocates_counter_when_unset() -> None:
    """A relation marker without an explicit counter gets a fresh monotonic one."""
...
def test_native_field_resolver_property_returns_own_resolver() -> None:
    """``NativeField.resolver`` returns the resolver passed at construction."""
    from graphql import GraphQLString
...
def test_native_field_resolver_property_none_by_default() -> None:
    """``NativeField.resolver`` is ``None`` when no resolver was supplied."""
    from graphql import GraphQLString
