...
Run: .venv/bin/python -m pytest tests/native/test_subscription_native_build.py -q
"""
...
def test_subscription_build_runs_with_graphene_blocked():
    """Define + mount + build a subscription with graphene blocked at meta_path.
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_subscription_root_sdl_action_enum_id_filters():
    ...
    enum_m = re.search(
        r"enum PostSubscriptionAction \{(.*?)\}", sdl, re.DOTALL
    )
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_subscription_payload_choices_renders_canonical_enum():
    """The subscription event payload renders a choices field as the shared Enum.
