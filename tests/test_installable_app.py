...
def test_package_is_installable_in_a_fresh_registry() -> None:
    ...
    assert result.returncode == 0, f"installability probe failed:\n{result.stderr}"
    assert "INSTALLABLE: True" in result.stdout


def test_app_is_installed_under_suite_settings() -> None:
...
def test_graphql_schema_command_is_discovered_when_app_installed() -> None:
