...
def _build_native_schema(
    pagination: "LimitOffsetGraphqlPagination | PageGraphqlPagination | None" = None,
    results_field_name: str | None = None,
) -> "tuple[GraphQLSchema, type[DjangoListObjectType]]":
    """Compile a native GraphQLSchema exposing a paginated, filtered Author list.

    The list type is built via "compile_native_root" (NOT graphene.Schema), so
    the returned schema is a plain "graphql.GraphQLSchema".

    Args:
        pagination: The paginator instance to attach to the list field's Meta,
            or None to leave pagination unset.
        results_field_name: A custom name for the results field, or None to
            keep the default "results" name.

    Returns:
        A tuple of the compiled schema and the generated list object type.
    """
    from graphql import GraphQLSchema
...
def test_compile_native_root_builds_paginated_list_field() -> None:
    """Ships broken if compile_native_root raises instead of building a
    DjangoListObjectField with a paginating container.

    Guards the WU5/WU6 NotImplementedError regression on the native path.
    """
...
@pytest.mark.django_db
def test_results_field_carries_pagination_args() -> None:
    """Ships broken if the container's results field is missing the
    paginator's args (limit/offset/ordering) on the compiled schema.
    """
    from django_graphex.paginations import LimitOffsetGraphqlPagination
...
@pytest.mark.django_db
def test_list_field_carries_filter_arg() -> None:
    """Ships broken if the list field is missing the native filter input arg
    introduced in WU3.
    """
...
@pytest.mark.django_db
def test_limit_actually_slices() -> None:
    """Ships broken if "limit: 5" stops actually slicing the result set and
    becomes a silent no-op instead.
    """
...
@pytest.mark.django_db
def test_offset_actually_slices() -> None:
    """Ships broken if the "offset" arg stops shifting the page window on the
    native pagination path.
    """
    from graphql import graphql_sync
...
@pytest.mark.django_db
def test_page_pagination_actually_slices() -> None:
    """Ships broken if PageGraphqlPagination's "page"/"pageSize" args stop
    slicing the result set on the native pagination path.
    """
    from graphql import graphql_sync
...
@pytest.mark.django_db
def test_filter_actually_filters() -> None:
    """Ships broken if the "filter" arg stops narrowing rows and becomes a
    silent no-op that returns everything.
    """
    from graphql import graphql_sync
...
@pytest.mark.django_db
def test_ordering_applies() -> None:
    """Ships broken if the "ordering" arg stops sorting the results on the
    native pagination path.
    """
    from graphql import graphql_sync
    ...
    result = graphql_sync(schema, '{ authors { results(ordering: "name") { name } } }')
    assert result.errors is None, result.errors
    names = [r["name"] for r in result.data["authors"]["results"]]
...
@pytest.mark.django_db
def test_relation_spanning_ordering_rejected() -> None:
    """Ships broken if a relation-spanning ordering term stops producing a
    GraphQL error, meaning "_validate_ordering_terms" no longer fires on the
    native path.
    """
    from graphql import graphql_sync
...
@pytest.mark.django_db
def test_renamed_results_field_name_flows_through() -> None:
    """Ships broken if a renamed results_field_name ("items") stops being both
    the wire key and the paginating field, so args and slicing keep following
    "results" instead of the renamed field.
    """
    from graphql import graphql_sync
    ...
    result = graphql_sync(schema, "{ authors { items(limit: 3) { name } totalCount } }")
    assert result.errors is None, result.errors
...
def test_native_pagination_field_is_dataclass_no_graphene() -> None:
    """Ships broken if NativePaginationField stops being a plain dataclass
    (type, paginator) with the list_resolver/wrap_resolve logic extracted
    from the graphene field.
    """
    import dataclasses
...
def test_native_pagination_field_already_paginated_no_double_slice() -> None:
    """Ships broken if already_paginated=True stops short-circuiting
    list_resolver, causing a double pagination slice on already-paginated
    rows.
    """
    ...
    class _SpyPaginator:
        """Fake paginator that records whether it was invoked."""

        __name__ = "Spy"
        called = False

        def paginate_queryset(self, qs: Any, **kwargs: Any) -> list[Any]:
            """Record the call and return only the first row.

            Args:
                qs: The queryset (or list) to "paginate".
                **kwargs: Ignored pagination keyword arguments.

            Returns:
                A one-item list built from the first element of "qs".
            """
            self.called = True
            return list(qs)[:1]
    ...
    assert paginator.called is False, (
        "paginator must NOT be called when already_paginated"
    )


def test_native_pagination_field_paginates_when_not_already() -> None:
    """Ships broken if already_paginated=False stops delegating to
    paginate_queryset, so rows would no longer be sliced by the resolver.
    """
    from django_graphex.base_types import DjangoListObjectBase
    from django_graphex.paginations.utils import NativePaginationField

    class _SlicePaginator:
        """Fake paginator that slices to the requested limit."""

        __name__ = "Slice"

        def paginate_queryset(self, qs: Any, **kwargs: Any) -> list[Any]:
            """Slice "qs" to the "limit" keyword argument (or its full length).

            Args:
                qs: The queryset (or list) to slice.
                **kwargs: Pagination keyword arguments; only "limit" is read.

            Returns:
                The first "limit" elements of "qs".
            """
