...
1. "ErrorType" (the mutation error payload "{field, messages}") becomes a
   NATIVE "ObjectType" ("django_graphex.core.base.ObjectType" subclass,
   ...
   instantiated at runtime by "nested.py" / "utils.py" / "core/backend.py".
...
def test_errortype_metaclass_is_pydantic_model_metaclass() -> None:
    """Assert that "type(ErrorType) is pydantic ModelMetaclass" (native-base identity).

    Ships broken if "ErrorType" stops being built on the native pydantic
    metaclass, which would mean it silently regressed onto a different base.
    """
...
def test_errortype_is_native_objecttype_subclass() -> None:
    """Assert that "ErrorType" subclasses the native "ObjectType" base, not graphene's.

    Ships broken if "ErrorType" stops being a subclass of the native
...
def test_errortype_is_not_graphene_objecttype() -> None:
    """Assert that "ErrorType" is NOT a graphene class (it left graphene).
...
def test_errors_module_has_no_graphene_import() -> None:
    """Assert that "errors.py" must NOT import graphene at module top level.
...
def test_errortype_value_object_ctor_roundtrip() -> None:
    """Assert that 'ErrorType(field=, messages=)' round-trips via attribute access.

    nested.py:360 reads "error.messages"; nested.py:358 reads "error.field"
    via getattr. core/backend.py:29 / utils.py:136 construct it. The native
...
def test_is_plain_object_type_recognises_native_errortype() -> None:
    """Assert that "_is_plain_object_type(ErrorType)" is True via the NATIVE marker.
    ...
    """
    from django_graphex.core.schema_compiler import _is_plain_object_type
    from django_graphex.errors import ErrorType
...
def test_is_plain_object_type_excludes_non_class() -> None:
    """Assert that non-class field types (wrappers / scalars) are not plain objects.

    Ships broken if "_is_plain_object_type" starts misclassifying a
    non-class graphql-core value (e.g. a scalar instance) as a plain object.
    """
...
def test_native_errortype_compiles_to_object_type_with_both_fields() -> None:
    ...
    from django_graphex.core.schema_compiler import _compile_wrapped_field_type
    from django_graphex.errors import ErrorType
