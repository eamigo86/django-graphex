...
def test_fully_native_root_schema_builds_with_graphene_blocked() -> None:
    """Assert that a fully-native schema compiles and prints SDL with graphene blocked.

    Covers query, mutation, subscription, and pagination roots, all
    graphene-free, with no ModuleNotFoundError and graphene never
    re-entering "sys.modules".

    If this fails, the native code path would still depend on graphene
    somewhere in query, mutation, subscription, or pagination compilation.
    """
    from graphql import (
        GraphQLArgument,
        GraphQLBoolean,
...
def test_native_wrapped_object_payload_and_declared_args_graphene_free() -> None:
    """Assert that the native wrapper and declared-arg compile paths import no graphene.

    Complements the schema build above: a payload field declared via a
    native "NativeList" / "NativeNonNull" around a plain native
    "ObjectType" (the ErrorType-style payload, S-ROOTS-c) AND a declared
    scalar field carrying native "args" must compile graphene-free —
    proving "_compile_wrapped_field_type"'s native-wrapper branch and the
    declared-arg converter import no graphene on the native path.

    If this fails, either the native list/non-null wrapper compilation or
    the declared-arg converter would still trigger a graphene import.
    """
    from graphql import (
        GraphQLArgument,
        GraphQLBoolean,
