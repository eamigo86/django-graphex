"""WU12 Phase-5 carry-forward coverage — core/schema_compiler.py.

Lifts "core/schema_compiler.py" branch coverage to >=95% by exercising the
field-dispatch arms the existing "tests/core/" suite did not reach: explicit
...
from graphql.execution import default_field_resolver  # noqa: E402
...
def test_rendered_field_name_honors_explicit_name() -> None:
    """Ships broken if an explicit "name=" stops being returned verbatim.

    An overridden wire name must win over the derived camelCase attribute
    name.
    """
    from django_graphex.core.schema_compiler import _rendered_field_name
...
def test_rendered_field_name_camelcases_when_no_explicit() -> None:
    """Ships broken if the absence of an explicit name stops camelCasing the
...
def test_is_subscription_field_false_for_wrong_class_name() -> None:
    """Ships broken if a field whose class is not "SubscriptionField" stops
    returning False.
    """
    from django_graphex.core.schema_compiler import _is_subscription_field
...
def test_is_subscription_field_checks_build_native_field_callable() -> None:
    """Ships broken if a "SubscriptionField" whose type has
    "_build_native_field" stops passing the gate.
    """
    from django_graphex.core.schema_compiler import _is_subscription_field

    class _Target:
        @staticmethod
        def _build_native_field() -> None:  # pragma: no cover - identity only
            return None
...
def test_resolver_for_returns_default_without_source() -> None:
    """Ships broken if "_resolver_for(None, None)" stops returning the
    graphql-core default resolver.
    """
    from django_graphex.core.schema_compiler import _resolver_for

    assert _resolver_for(None, None) is default_field_resolver
    assert _resolver_for(None, "field") is default_field_resolver


def test_resolver_for_recovers_resolve_method() -> None:
    """Ships broken if "_resolver_for" stops returning the source's
    "resolve_<name>" when declared.
    """
    from graphql import GraphQLString

    from django_graphex.core import ObjectType
    from django_graphex.core import field as native_field
    from django_graphex.core.schema_compiler import _resolver_for
...
def test_build_scalar_field_converts_args() -> None:
    """Ships broken if a native scalar field declaring args stops converting
    them to GraphQLArgument.
...
def test_compile_declared_field_wrapped_django_output_list() -> None:
    """Ships broken if a native "field(NativeList(DjangoObjectType))" declared
    field stops compiling to a list of the canonical container/node native
    type (wrapper preservation).
...
@pytest.mark.django_db
def test_compile_declared_field_with_args() -> None:
    """Ships broken if a native declared field with args stops converting
    them.
...
@pytest.mark.django_db
def test_build_plain_object_field_with_args() -> None:
    """Ships broken if a native plain-ObjectType field declaring args stops
    converting them.
    ...
    from django_graphex.core import ObjectType
    from django_graphex.core import field as native_field
    from django_graphex.core.schema_compiler import _build_plain_object_field
    ...
    gql_field = _build_plain_object_field(
        field, source_cls=_Holder, field_name="nested"
    )
...
@pytest.mark.django_db
def test_filter_arg_empty_without_declared_fields() -> None:
    """Ships broken if "_filter_arg" stops returning an empty dict when the
    field declares no fields or custom filters.
    """
    from django_graphex.core.schema_compiler import _filter_arg
...
def test_unwrap_to_node_type_peels_graphql_core_wrappers() -> None:
    """Ships broken if a field whose ".type" is
    "GraphQLList(GraphQLNonNull(node))" stops unwrapping to the node.
    """
    from django_graphex.core.schema_compiler import _unwrap_to_node_type
...
def _scalar_field() -> GraphQLField:
    """Build a plain GraphQLField(GraphQLString) for wrapper-unwrap fixtures.

    Returns:
        field: A minimal scalar GraphQLField used as a leaf-type placeholder.
    """
...
@pytest.mark.django_db
def test_build_filter_paginate_list_field_inner_non_null_and_args() -> None:
    """Ships broken if DjangoFilterPaginateListField stops compiling to
    "[Node!]" with pagination args mounted on the field.
    """
...
def test_compile_native_root_none_returns_none() -> None:
    """Ships broken if "compile_native_root(None, ...)" stops returning
    None.
    """
    from django_graphex.core.schema_compiler import compile_native_root
...
def test_compile_native_root_builds_subscription_field() -> None:
    """Ships broken if a SubscriptionField on the root stops compiling via
    "_build_native_field".
    ...
    class _SubTarget:
        @staticmethod
        def _build_native_field() -> GraphQLField:
            return built
