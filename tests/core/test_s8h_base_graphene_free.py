# -*- coding: utf-8 -*-
"""S8h (graphene-removal): core/base.py off graphene — the LAST top-level import.

"core/base.py" held the final two TOP-LEVEL graphene imports in the entire
...
This module proves:
  1. core/base.py has ZERO top-level graphene imports (AST probe).
...
def test_no_toplevel_graphene_import_in_native_base() -> None:
    """Assert that core/base.py has ZERO top-level graphene imports (AST probe).

    Ships broken if a top-level "import graphene" (or "from graphene...")
    reappears at "core/base.py" module level.
    """
    hits = _toplevel_graphene_imports(_BASE_PATH)
    assert hits == [], (
        "core/base.py still has top-level graphene imports "
        f"(uninstall-blocking): {hits}"
    )


def test_zero_toplevel_graphene_imports_in_whole_package() -> None:
    ...
    assert hits == [], (
        f"django_graphex still has top-level graphene imports somewhere: {hits}"
    )


def test_descriptor_tuples_keep_native_currency_drop_graphene() -> None:
...
def test_native_field_declaration_lands_in_meta_fields() -> None:
