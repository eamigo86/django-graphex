...
def _all_graphene_imports(module: ModuleType) -> list[str]:
...
def test_fields_module_is_fully_graphene_free() -> None:
    """Ships broken if "fields.py" reintroduces any graphene import anywhere
    (the S8c headline gate).
    """
...
def test_field_class_is_not_a_graphene_field_subclass(cls_name: str) -> None:
    """Ships broken if any field class regains graphene "Field" in its MRO.

    Args:
        cls_name: The name of the field class to look up on
            "django_graphex.fields" and check, parametrized over every S8c
            field kind.
    ...
    graphene_bases = [
        b.__qualname__ for b in cls.__mro__ if b.__module__.split(".")[0] == "graphene"
    ]
...
def test_field_instances_expose_duck_typed_contract() -> None:
    """Ships broken if a built field INSTANCE stops exposing ".type" /
    ".args" / ".wrap_resolve" (or the other read-contract attributes).
...
def test_types_no_top_level_graphene_field_inputfield_int() -> None:
    """Ships broken if "types.py" starts importing graphene "Field",
    "InputField", or "Int" again at top level.
    """
...
def test_mutation_no_top_level_graphene_field() -> None:
    """Ships broken if "mutation.py" starts importing graphene "Field" again
    at top level.
    """
...
def test_every_field_kind_present_in_compiled_sdl() -> None:
    """Ships broken if any S8c field kind stops rendering into the SDL (a
    silent drop).
    """
...
def test_paginate_list_field_keeps_nonnull_inner_shape() -> None:
    """Ships broken if "DjangoFilterPaginateListField" stops keeping the
    "[Node!]" inner-non-null shape.
    """
    sdl = _build_field_kind_schema()
