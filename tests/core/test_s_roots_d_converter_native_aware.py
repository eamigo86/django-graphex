...
/ "String" for Binary / …). Those names come from "core/scalars.py" via the
...
# ---------------------------------------------------------------------------
def test_construct_fields_skips_dead_scalar_descriptors_on_native() -> None:
...
# ---------------------------------------------------------------------------
def test_construct_fields_keeps_gfk_descriptor_on_native() -> None:
    """Assert that the GFK converter descriptor (Dynamic) is STILL built on native.

    Ships broken if the per-field-type skip introduced for scalars starts
    blanket-dropping the generic-foreign-key relation descriptor too.
    """
    from django_graphex.types import DjangoObjectType
...
def test_construct_fields_keeps_relation_descriptors_on_native() -> None:
    """Assert that relation Dynamic descriptors (FK / M2M / reverse FK) are KEPT.

    Ships broken if the per-field-type scalar skip starts dropping relation
    descriptors it must never touch.
    """
    from django_graphex.types import DjangoObjectType
...
# ---------------------------------------------------------------------------
def test_native_scalar_sdl_is_byte_identical_after_skip() -> None:
    """The native SDL scalar names must be UNCHANGED after the converter skip.

    Byte-identical to the captured baseline (built from core/scalars.py via the
...
# ---------------------------------------------------------------------------
def test_silent_drop_guard_all_field_kinds_present() -> None:
    """Compile a scalar + date + binary + uuid + json + FK model and check fields.

    Asserts ALL expected fields are present in the compiled output type with
    correct types.
...
def test_silent_drop_guard_relation_list_containers_present() -> None:
    """Assert that a model with FK + M2M + reverse FK keeps all relation containers.

    Ships broken if any relation field (author FK, category FK, tags M2M,
    co_authors M2M, comments reverse FK) silently vanishes from the compiled
    output type.
    """
...
# ---------------------------------------------------------------------------
def test_base_types_scalar_classes_kept_documented() -> None:
