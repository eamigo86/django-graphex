...
def test_converter_has_no_top_level_graphene_import() -> None:
    """Ships broken if "converter.py" reintroduces a top-level graphene
    import, blocking the graphene uninstall (S8i).
    """
...
def test_converter_module_body_is_graphene_token_free() -> None:
    """Ships broken if a graphene import/name reappears at converter.py
    MODULE level.
...
def test_choices_converter_off_graphene_on_both_paths() -> None:
    """Ships broken if the choices converter path regains a graphene
    dependency on either OUTPUT or INPUT.
...
def test_fk_and_m2m_output_convert_to_native_marker() -> None:
    """Ships broken if the to-ONE FK (S-rel-2) or to-MANY M2M (S-rel-3)
    OUTPUT converters stop returning native markers.
...
def test_fk_input_converts_to_native_marker() -> None:
    """Ships broken if the FK INPUT converter stops returning a graphene-free
    native marker (S-input-5).
...
def test_converter_imports_under_native_backend() -> None:
    """Ships broken if importing converter under native starts requiring
    graphene at module load time.
