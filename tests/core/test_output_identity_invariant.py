...
def test_compile_all_outputs_relation_identity_no_string_fallback() -> None:
    """Cross-referencing DjangoObjectTypes must resolve relations to one instance.

    Contract: relation resolution ships broken if the FK thunk target is not
    identical to the related type's GraphQLObjectType instance (a String
    fallback or a second copy would both silently corrupt the schema).
...
@pytest.mark.django_db
def test_compile_all_outputs_self_cycle_resolves_to_same_instance() -> None:
    """A self-referencing FK must resolve to one instance with no recursion error.

    Contract: cycle handling ships broken if a self-referencing FK
    (A.parent -> A) raises RecursionError or falls back to GraphQLString
    instead of resolving to the same node type instance.
...
@pytest.mark.django_db
def test_compile_all_outputs_single_instance_stable_across_recompile() -> None:
    """Repeated compiles must not create a second instance for a registered type.

    Contract: recompilation ships broken if compile_all_outputs() replaces
    the pinned GraphQLObjectType instance instead of reusing it, since a
    mutation may have already captured the original reference.
...
@pytest.mark.django_db
def test_schema_assembly_no_duplicate_type_name() -> None:
    """Reviewer probe 3 — the permanent regression guard for the identity bug.

    Contract: schema assembly ships broken if two cross-referencing
    DjangoObjectTypes plus a mutation referencing one of them produce two
    distinct GraphQLObjectType instances sharing one name, since
    GraphQLSchema raises a duplicate-name TypeError in that case.
