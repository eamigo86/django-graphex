...
"error" declared via "field()"), arguments declared via "class Arguments", and a
...
# --------------------------------------------------------------------------- #
def test_native_mutation_is_native_objecttype_subclass() -> None:
    """Assert that "Mutation" is a subclass of the native "ObjectType" base.

    This is so its output "field()" descriptors compile into a payload object
    type and the "cls(**resp)" value-object round-trip (the silent-null
    safety net) works.
    """
...
def test_native_mutation_output_fields_land_in_meta_fields() -> None:
    """Assert that "field()" output descriptors on a "Mutation" subclass land
    in "_meta.fields" (the silent-drop epicenter) alongside no spurious entries.
    """
...
def test_native_mutation_payload_value_object_roundtrips() -> None:
    """Assert that 'cls(ok=True, message="pong")' stashes the values as attrs.

    The payload values become instance attributes (the S6c silent-null fix)
    — they are NOT dropped by Pydantic.
    """
    from graphql import GraphQLBoolean, GraphQLString
...
# --------------------------------------------------------------------------- #
# 2. class Arguments -> {name: GraphQLArgument} on the field                     #
# --------------------------------------------------------------------------- #
def test_native_mutation_field_args_compile_from_class_args() -> None:
    """Assert that "class Arguments" declarations compile to "GraphQLArgument"s.

    They land on the returned field via the native arg form.
    """
    from graphql import (
        GraphQLArgument,
        GraphQLBoolean,
...
# --------------------------------------------------------------------------- #
def test_native_mutation_field_output_is_payload_object_type() -> None:
    """Assert that "Mutation.Field()" output type is the compiled PAYLOAD type.

    The compiled "GraphQLObjectType" carries the declared output fields — NOT
    the Phase-4 "GraphQLString" placeholder.
    """
    from graphql import (
        GraphQLBoolean,
        GraphQLField,
...
def test_native_mutation_field_registered_in_identity_set() -> None:
    """Assert that "Mutation.Field()" registers its GraphQLField id.

    Registers into "_NATIVE_FIELD_IDENTITIES" so "_collect_root_attrs"
    recovers it (and never silently drops it) when mounted on a root.
    """
...
# --------------------------------------------------------------------------- #
def test_native_objecttype_accepts_raw_graphql_field_attr() -> None:
...
def test_native_mutation_compiles_and_executes_nonnull_payload() -> None:
    """CARDINAL: a hand-written native "Mutation" mounted on a native root.

    The "ObjectType" root compiles via "compile_native_root" (field PRESENT
    in the SDL — silent-drop guard) and EXECUTES via "graphql_sync" returning
    a NON-NULL payload ("ok" / a nested object / "error") — the silent-null
    ...
    document = 'mutation { createAuthor(name: "Grace") { ok author { name } error } }'
...
@pytest.mark.django_db
def test_native_mutation_error_path_roundtrips() -> None:
    ...
    result = graphql_sync(schema, 'mutation { doFail(name: "dup") { ok error } }')
