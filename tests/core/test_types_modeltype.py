...
def test_django_model_type_full_public_api() -> None:
    """Ships broken if DjangoModelType stops exposing its full public API
    under native.
    """
...
@pytest.mark.django_db
def test_django_model_type_output_type_accessible() -> None:
    """Ships broken if "DjangoModelType._meta.output_type" stops being
    accessible as a DjangoObjectType.
    """
...
@pytest.mark.django_db
def test_django_model_type_output_list_type_accessible() -> None:
    """Ships broken if "DjangoModelType._meta.output_list_type" stops being
    accessible.
    """
...
def test_django_model_type_create_field_returns_graphql_field() -> None:
    """Ships broken if "CreateField()"/"UpdateField()" stop returning a
    GraphQLField (WU-3).

    Phase 4 WU-3 implements "DjangoModelType.*Field()" native branches. Under
    native, "CreateField()" and "UpdateField()" return graphql-core
    GraphQLField instances (not graphene Field), so no ValueError is raised.
...
@pytest.mark.django_db
def test_django_model_type_model_meta() -> None:
    """Ships broken if "DjangoModelType._meta.model" stops being accessible
    under native.
    """
    from django_graphex.types import DjangoModelType
    from tests.models import Category
