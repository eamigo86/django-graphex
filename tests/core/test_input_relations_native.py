...
def test_input_build_does_not_import_graphene() -> None:
    """Protect that building create + update "DjangoInputObjectType"s stays graphene-free.

    Covers a model with FK + O2O + M2M + reverse + choices.
    ...
    This asserts the input types build + their thunks evaluate without error. If
    this regresses, a graphene import could sneak back into the INPUT class-def
    path and reintroduce the dependency this slice removed.
    """
    reg = Registry()
...
@pytest.mark.django_db
def test_full_class_def_output_and_input_graphene_free() -> None:
    """Protect that the FULL class-def stays graphene-free end to end.

    Defining a model's OUTPUT type AND its create/update INPUT types must
    build without importing graphene.
    ...
    If this regresses, either surface could reintroduce a graphene dependency
    that this slice was written to remove.
    """
    reg = Registry()
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_input_relation_sdl_parity() -> None:
    """Protect INPUT-side relation SDL: FK "id: ID" (create-required "ID!"); M2M / reverse "[ID!]".

    If this regresses, relation fields on the compiled INPUT type could
    render with the wrong nullability or lose their list wrapping, silently
    changing what a create/update mutation accepts.
    """
    reg = Registry()
    ...
    assert isinstance(create_fields["author"].type, GraphQLNonNull), (
        f"required FK on create must be ID!, got {create_fields['author'].type!r}"
    )
    assert update_fields["author"].type is GraphQLID, (
        f"FK on update must be a nullable ID, got {update_fields['author'].type!r}"
    )
...
@pytest.mark.django_db
def test_choices_input_uses_shared_enum() -> None:
    """Protect that the choices INPUT field reuses the shared enum, not a String.

    The choices INPUT field must render as the native "GraphQLEnumType" (the
    SAME shared enum the OUTPUT path uses), NOT "GraphQLString". If this
    regresses, the INPUT and OUTPUT sides could drift onto two different
    enum instances (or the input side could fall back to a plain string),
    letting invalid choice values slip past GraphQL-level validation.
    """
    ...
    shared = build_choices_enum_type(EnumCollisionItemA._meta.get_field("status"), reg)
...
# --------------------------------------------------------------------------- #
def test_graphene_marker_branch_retired() -> None:
    """Part 3 retirement: protect that "types._graphene_descriptor_markers" is GONE.
    ...
    Part B from #1609 / S-enum-2). If this regresses, the dead helper would be
    reintroduced, undoing the retirement this test locks in.
    """
...
@pytest.mark.django_db
def test_comprehensive_build_reaches_yank_fields_graphene_free() -> None:
    """Protect that a comprehensive native build reaches "_yank_fields" graphene-free.
    ...
    Part 3 STEP 0-B proof that authorizes retiring the marker branch). If
    this regresses, a graphene descriptor would once again reach
    "_yank_fields", which is exactly the condition that blocked retiring the
    marker branch.
    """
    import django_graphex.types as types_mod
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_choices_input_field_dropped_when_excluded() -> None:
    """Protect exclude_fields gating for the choices INPUT field.

    A choices field named in "Meta.exclude_fields" must NOT land on the
    compiled INPUT type (issue #65; the leak S-input-5 introduced). If this
    regresses, a field the model author explicitly excluded would leak back
...
@pytest.mark.django_db
def test_choices_input_field_dropped_when_not_in_only() -> None:
    """Protect only_fields gating for the choices INPUT field.

    A choices field absent from "Meta.only_fields" must NOT land on the
    compiled INPUT type (issue #65; the leak S-input-5 introduced). If this
    regresses, a field the model author did not opt in via only_fields would
    leak onto the mutation input anyway.
    """
    reg = Registry()
...
@pytest.mark.django_db
def test_choices_input_field_force_included_overrides_only() -> None:
    """Protect include_fields overriding only_fields for the choices INPUT field.

    "Meta.include_fields" force-includes a choices field even when
    "only_fields" would otherwise drop it — EXACT base-loop semantics. If
    this regresses, include_fields would stop acting as the documented
    override for a field only_fields would otherwise exclude.
    """
    reg = Registry()
...
@pytest.mark.django_db
def test_choices_input_field_present_without_field_selection() -> None:
    """Control: protect that the choices field stays present with no field selection.

    With no only/exclude/include the choices field stays present (guards the
    gating from over-dropping). If this regresses, the field-selection
    gating logic would be dropping fields even when the model author never
    applied any selection at all.
    """
    reg = Registry()
...
def test_relation_input_field_dropped_when_excluded(
    out_name: str, wire_name: str
) -> None:
    """Protect exclude_fields gating for relation (ID-surface) INPUT fields.

    Args:
        out_name: The snake_case field name passed to Meta.exclude_fields.
        wire_name: The camelCase wire alias expected to also be absent.

    A relation named in "Meta.exclude_fields" (by its snake out_name) must
    NOT land on the compiled INPUT type — neither the wire alias nor the
    snake accessor (audit rank 1). If this regresses, a client could submit
    hidden relation data through a field the model author explicitly
    excluded — an access-control hole.
    """
    reg = Registry()
...
def test_relation_input_field_dropped_when_not_in_only(
    out_name: str, wire_name: str
) -> None:
    """Protect only_fields gating for relation (ID-surface) INPUT fields.

    Args:
        out_name: The snake_case field name expected to be absent (not
            listed in Meta.only_fields).
        wire_name: The camelCase wire alias expected to also be absent.

    A relation absent from "Meta.only_fields" must NOT land on the compiled
    INPUT type (audit rank 1). If this regresses, a relation not opted into
    only_fields would still leak onto the mutation input, bypassing the
    field-selection allowlist.
    """
    reg = Registry()
...
def test_relation_input_field_force_included_overrides_only(
    out_name: str, wire_name: str, is_list: bool
) -> None:
    """Protect include_fields overriding only_fields for relation INPUT fields.

    Args:
        out_name: The snake_case field name passed to Meta.include_fields.
        wire_name: The camelCase wire alias expected to be present.
        is_list: Whether the relation is list-valued (M2M) versus a single FK.

    "Meta.include_fields" force-includes a relation even when "only_fields"
    would otherwise drop it — EXACT base/choices-loop semantics (audit rank
    1). If this regresses, include_fields would stop acting as the
    documented override for a relation only_fields would otherwise exclude.
    """
    reg = Registry()
    ...
    if is_list:
        assert isinstance(ftype, GraphQLList), (
            f"force-included M2M {out_name!r} must still be a [ID!] list, got {ftype!r}"
        )
        assert ftype.of_type.of_type is GraphQLID
    else:
...
def test_nested_input_field_dropped_when_excluded() -> None:
    ...
    the compiled INPUT type (audit rank 2). If this regresses, a nested
    input the model author explicitly excluded would leak back onto the
    mutation input.
    """
    reg = Registry()
...
@pytest.mark.django_db
def test_nested_input_field_dropped_when_not_in_only() -> None:
    """Protect only_fields gating for nested object-input fields.

    A nested object-input absent from "Meta.only_fields" must NOT land on
    the compiled INPUT type (audit rank 2). If this regresses, a nested
    input not opted into only_fields would still leak onto the mutation
...
@pytest.mark.django_db
def test_nested_input_field_force_included_overrides_only() -> None:
    """Protect include_fields overriding only_fields for nested object-input fields.

    "Meta.include_fields" force-includes a nested object-input even when
    "only_fields" would otherwise drop it (audit rank 2). If this regresses,
    include_fields would stop acting as the documented override for a nested
...
@pytest.mark.django_db
def test_nested_input_field_present_without_field_selection() -> None:
    """Control: protect that the nested object-input stays present with no field selection.

    With no only/exclude/include the nested object-input stays present
    (guards the gating from over-dropping). If this regresses, the
    field-selection gating logic would be dropping the nested input even
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_self_ref_o2o_present_in_create_input() -> None:
    """Protect the self-ref O2O silent-drop canary on the INPUT side.

    The self-ref O2O "spouse" stays present in the create-input
    "_meta.fields" (the issue #52 silent-drop canary, INPUT side). If this
    regresses, a self-referential relation could silently disappear from
    the compiled input, reintroducing the issue #52 data-loss bug.
    """
    reg = Registry()
...
@pytest.mark.django_db
def test_post_input_relation_presence_preserved() -> None:
    """Protect broad relation-presence preservation on the create-input.

    FK + M2M + reverse relations stay present in the create-input
    "_meta.fields" (broad silent-drop guard) and are native markers. If this
    regresses, any of these relation kinds could silently disappear from
    the compiled input's field metadata.
    """
    reg = Registry()
