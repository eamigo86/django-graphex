...
def test_native_filter_input_collision_multi_path_same_model() -> None:
    """Ships broken if a model exposed via both nested and flat-paginate filter
    paths, with a custom "@filter_field", raises a duplicate-name TypeError on
    schema assembly instead of both filter inputs sharing the custom filter.

    Pre-fix: the nested/relation path drops "search", producing two distinct
    "OrderChildFilterInput" shapes and a "Schema must contain uniquely named
    types but contains multiple types named 'OrderChildFilterInput'" error.
    """
...
@pytest.mark.django_db
def test_native_nested_field_carries_custom_filters() -> None:
    """Ships broken if "DjangoNestedListObjectField" stops propagating the node
...
@pytest.mark.django_db
def test_native_custom_filter_search_executes_end_to_end() -> None:
    """Ships broken if a real "filter: { search: ... }" custom-filter query over
    a flat-paginate field stops executing, or stops actually filtering rows,
...
@pytest.mark.django_db
def test_native_single_filter_path_name_unchanged_byte_parity() -> None:
    """Ships broken if a model with a single filter shape stops rendering its
    filter input as exactly "<Model>FilterInput" (PascalCase), i.e. if the
    collision fix starts adding a suffix/disambiguator for the common
    single-path case (#1533).
    """
    ...
    # The name must be EXACTLY 'OrderParentFilterInput' (PascalCase) — no
    # suffix/discriminator for the single-shape case.
    assert "OrderParentFilterInput" in type_map, (
        "single-shape filter input must use the PascalCase name "
