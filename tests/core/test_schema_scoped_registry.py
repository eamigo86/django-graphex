...
Run: .venv/bin/python -m pytest -q \
    tests/core/test_schema_scoped_registry.py
"""
...
class _FakeSchema:
    ...
    def __init__(self, extensions: dict | None) -> None:
        """Store the fake extensions mapping.

        Args:
            extensions: The extensions dict to expose, or None.
        """
...
class _FakeInfo:
    ...
    def __init__(self, schema: object | None) -> None:
        """Store the fake schema object.

        Args:
            schema: The schema-like object to expose, or None.
        """
        self.schema = schema
...
def test_schema_sets_gdx_registry_extension_to_default_pair() -> None:
    """Ships broken if a default "DjangoGraphQLSchema" stops exposing the
    GLOBAL pair on the extension.
...
@pytest.mark.django_db
def test_schema_keeps_protected_fields_extension_alongside_registry() -> None:
    """Ships broken if "gdx_registry" starts clobbering "gdx_protected_fields"
...
@pytest.mark.django_db
def test_explicit_registries_kwarg_is_used_for_extension() -> None:
    """Ships broken if an explicit "registries=" pair stops being the one
    exposed on the extension.
...
# =========================================================================== #
@pytest.mark.django_db
def test_resolve_type_prefers_schema_scoped_registry_over_class_binding() -> None:
    """Ships broken if resolve_type stops reading
    ...
    class PaymentUnion(DjangoUnionType):
        class Meta:
            types = (AccountTypeA, InvoiceTypeA)
            registry = reg_a
...
@pytest.mark.django_db
def test_resolve_type_falls_back_to_class_binding_when_schema_scope_misses() -> None:
    """Ships broken if resolution stops falling back to "_dgx_registry" when
    the schema-scoped registry has NO type.
...
@pytest.mark.django_db
def test_resolve_type_falls_back_when_info_has_no_gdx_registry() -> None:
    """Ships broken if resolve_type with "info=None" (or no extension) stops
    using the class chain.
...
@pytest.mark.django_db
def test_resolve_type_raises_on_total_miss_never_returns_none() -> None:
    """Ships broken if an instance unresolvable in EVERY candidate registry
    stops RAISING (returning None instead).
    ...
    info = _FakeInfo(
        _FakeSchema({"gdx_registry": SchemaRegistries(graphene=Registry())})
    )
...
@pytest.mark.django_db
def test_interface_resolve_type_reads_schema_scope_too() -> None:
    """Ships broken if the interface resolve_type path stops sharing the
    schema-scoped read.
