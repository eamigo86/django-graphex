...
def test_types_pagination_build_has_no_graphene_branch() -> None:
    """ "types.py" must not build a graphene pagination container at class-def time.

    Contract: the native pagination migration ships broken if any graphene
    container factory ("get_pagination_field" / "get_page_info_field") is
    still reachable from "__init_subclass_with_meta__".
    ...
    source = inspect.getsource(
        types_mod.DjangoListObjectType.__init_subclass_with_meta__
    )
...
def test_to_graphql_fields_is_native_only() -> None:
    ...
    Contract: paginator argument compilation ships broken if any
    "to_graphql_fields" implementation still calls the lazy graphene
    accessor "_g()" or returns anything other than GraphQLArgument.
...
def test_pagination_factories_are_native_and_graphene_free() -> None:
    """Build a schema with limit/offset AND cursor paginated list fields.

    Contract: the pagination container build ships broken if any deleted
    graphene factory ("_g", "_build_generic_pagination_field",
    "_build_cursor_page_info", "CursorPageInfo", "GenericPaginationField")
    resurfaces, or the compiled SDL loses the native containers/pageInfo.
...
def test_native_pagination_build_does_not_import_graphene_subprocess() -> None:
    """Build a limit/offset AND cursor schema in a subprocess with graphene blocked.

    Contract: the native pagination build ships broken if it raises
    ModuleNotFoundError once "sys.meta_path" blocks any further graphene
    import — i.e. the pagination machinery must not lazily import graphene.

    NOTE: graphene may still enter "sys.modules" via a TRANSITIVE
    dependency at "import django_graphex" time (e.g. "core/descriptors.py"
    ...
    code = textwrap.dedent(
        ...
        from django_graphex.fields import DjangoListObjectField
        from django_graphex.core import ObjectType
        from django_graphex.paginations import CursorGraphqlPagination, LimitOffsetGraphqlPagination
        from django_graphex.types import DjangoListObjectType
        from django_graphex.schema import DjangoGraphQLSchema
        from tests.models import BasicModel
        from graphql import print_schema
...
def test_limit_offset_container_sdl_byte_identical() -> None:
    """The limit/offset "<Model>ListType" container SDL block matches the expected shape.

    Contract: the native SDL renderer ships broken if the limit/offset
    container drifts from the byte-for-byte expected shape (results with
    limit/offset/ordering args + totalCount, NO pageInfo). The element node
    type is normalized via "_normalize_results_element".
    """
...
def test_cursor_container_sdl_byte_identical() -> None:
    """The cursor "<Model>ListType" container SDL block matches the expected shape.

    Contract: the native SDL renderer ships broken if the cursor container
    drifts from the byte-for-byte expected shape (results with first/cursor
    args + totalCount + pageInfo: CursorPageInfo). The element node type is
    normalized via "_normalize_results_element".
    """
    _schema, sdl = _build_paginated_schema()
...
def test_limit_offset_pagination_resolves_native() -> None:
    ...
    Contract: limit/offset pagination ships broken if the sliced "results"
    or the "totalCount" count diverges from the underlying queryset.
    """
...
def test_cursor_pagination_resolves_native_with_page_info() -> None:
    ...
    # First page (first: 4) -> M00..M03, hasNextPage True, hasPreviousPage False.
    q = (
        "{ cursor { totalCount "
    ...
    # Second page using endCursor -> the next four rows, hasPreviousPage True.
    end_cursor = page_info["endCursor"]
    # Sanity: the end cursor decodes to the 4th row's ACTUAL ordering value.
    # The first row's auto-increment id is not guaranteed to be 1 (a committed
    # row in an earlier transactional test advances the sqlite sequence), so the
    # boundary is read from the returned rows rather than assumed to be id=4.
    fourth_row_id = data["results"][3]["id"]
    assert CursorGraphqlPagination.decode_cursor(end_cursor) == str(fourth_row_id)
    q2 = (
        "{ cursor { "
