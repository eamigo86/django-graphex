...
tests/core/test_wu9_mutation_execution.py and bugfix #1506).
...
- "_build_native_mutation_field" compiles "cls" (the wrapper) via
  "_compile_plain_object_type" into the payload GraphQLObjectType (not the bare
...
def _build_post_modeltype_schema() -> tuple[DjangoGraphQLSchema, type[DjangoModelType]]:
    """Build a native DjangoGraphQLSchema with a DjangoModelType Post mutation.

    Returns:
        pair: The assembled DjangoGraphQLSchema and the DjangoModelType class
            used to build its mutation fields.
    """
...
def test_modeltype_mutation_field_registered_and_mounted() -> None:
    """Ships broken if the DjangoModelType mutation field stops being
    registered in "_NATIVE_FIELD_REGISTRY" and mounted on the native
    Mutation root.
...
@pytest.mark.django_db
def test_modeltype_mutation_field_output_is_payload_not_node() -> None:
    """Ships broken if the field output type stops being the payload wrapper
    (ok/errors + output field) and reverts to the bare model node — must
    match graphene's "...: <ThisType>".
    """
...
@pytest.mark.django_db
def test_modeltype_mutation_field_arg_is_camelcase_wire_name() -> None:
    """Ships broken if the create input arg stops using the camelCase wire
    name ("newPost") with "out_name" set to the snake Python kwarg
    ("new_post").
    """
...
@pytest.mark.django_db
def test_modeltype_create_mutation_executes_and_creates_row() -> None:
    """Ships broken if a native DjangoModelType create mutation stops
    executing via graphql_sync, creating a real DB row, and returning the
    graphene-matching payload.
    """
    ...
    document = (
        ...
        "{ post { title author { name } } ok errors { field messages } } }" % author.id
    )
...
@pytest.mark.django_db
def test_modeltype_update_mutation_executes_and_updates_row() -> None:
    """Ships broken if a native DjangoModelType update mutation stops
    executing via graphql_sync, mutating a real DB row, and returning the
    typed payload reflecting the change.
    """
    from django.test import RequestFactory
    from graphql import graphql_sync
