...
def test_django_object_type_native_compiles_graphql_output_type() -> None:
    """Ships broken if a DjangoObjectType subclass stops setting
    "_meta.graphql_output_type" to a GraphQLObjectType.
    """
...
@pytest.mark.django_db
def test_django_object_type_native_extensions_gdx() -> None:
    """Ships broken if the compiled GraphQLObjectType stops carrying
    "extensions['gdx']" as a GdxPayload.
    """
    ...
    assert "gdx" in ext, "Compiled GraphQLObjectType must carry extensions['gdx']"
...
@pytest.mark.django_db
def test_django_object_type_honest_metaclass() -> None:
    """Ships broken if the Phase-7 S6b metaclass invariant regresses (the
    boundary this slice crosses).
    ...
    assert isinstance(
        _HonestMetaclassType._meta.graphql_output_type, GraphQLObjectType
    ), (
...
def test_polymorphic_types_reparented_onto_native_base() -> None:
    """Ships broken if the S6d structural invariant regresses: the polymorphic
    bases "DjangoUnionType" / "DjangoInterfaceType" must stay re-parented off
    graphene onto "native.base.ObjectType", whose sole metaclass is pydantic's
    "ModelMetaclass" (#1452 invariant).

    Mirrors the DjangoObjectType analog in
    "test_django_object_type_honest_metaclass" — asserts the positive native
...
@pytest.mark.django_db
def test_django_object_type_native_fields_include_scalars() -> None:
    """Ships broken if the compiled GraphQLObjectType stops including scalar
...
@pytest.mark.django_db
def test_django_object_type_native_meta_model() -> None:
    """Ships broken if "_meta.model" stops being accessible after native
    compile.
    """
    from django_graphex.types import DjangoObjectType
    from tests.models import Category
