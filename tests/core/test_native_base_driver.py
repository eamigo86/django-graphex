...
reproduced graphene-free on "django_graphex.core.base.ObjectType" so that a
later slice (S6b-S6f) can re-parent the 8 public "Django*" types onto the
native base WITHOUT graphene's metaclass.

S6a touches ONLY "core/base.py" — NO public type is re-parented yet, so the
...
def test_object_subclass_keeps_pydantic_model_metaclass() -> None:
    """Assert that a native ObjectType subclass keeps "type() is ModelMetaclass" (#1452).

    If this fails, the driver would require a custom metaclass, breaking
    the systemic metaclass-identity constraint that later re-parenting
    slices rely on.
    """
...
def test_meta_class_form_dispatches_options() -> None:
    """Assert that a class-style "Meta" is turned into kwargs and dispatched.

    If this fails, class-style Meta attributes would not reach
    "__init_subclass_with_meta__" as keyword options.
    """
    from django_graphex.core.base import ObjectType
...
def test_meta_dict_form_dispatches_options() -> None:
    """Assert that a dict-style "Meta" is dispatched identically to the class form.
    ...
    supports the dict FORM faithfully (graphene parity) — the dict just has
    to be declared in a Pydantic-safe way, i.e. ClassVar-annotated. No
    production type uses dict-form "Meta"; all 8 public types use the class
    form.

    If this fails, the dict form of Meta would either crash class creation
    or fail to dispatch its options.
    """
...
def test_meta_deleted_after_dispatch() -> None:
    """Assert that "Meta" is removed from the class after dispatch (graphene parity).

    If this fails, the inner Meta class would linger on the produced
    class, leaking an implementation detail into the public class body.
    """
    from django_graphex.core.base import ObjectType
...
def test_invalid_meta_type_raises() -> None:
    """Assert that a "Meta" that is neither a class nor a dict raises (graphene parity).

    If this fails, a malformed Meta declaration would be silently ignored
    instead of failing loudly at class-creation time.
    """
    from django_graphex.core.base import ObjectType
...
def test_abstract_base_skips_dispatch() -> None:
    """Assert that an abstract base does NOT invoke "__init_subclass_with_meta__".

    If this fails, declaring an abstract intermediate base would
    spuriously run driver logic meant only for concrete leaf subclasses.
    """
    from django_graphex.core.base import ObjectType
...
def test_no_meta_dispatches_with_empty_options() -> None:
    """Assert that a subclass with no "Meta" still dispatches, with empty options.

    If this fails, omitting Meta entirely would skip dispatch instead of
    calling the driver with an empty options mapping.
    """
    from django_graphex.core.base import ObjectType
...
def test_base_defining_driver_does_not_run_it_on_itself() -> None:
    """Assert that a non-abstract base defining a driver does NOT run it on itself.
    ...
    the driver a class declares runs for its SUBCLASSES, never for the
    class that declares it. "DjangoObjectType" / "DjangoListObjectType"
    rely on this — they define a driver that asserts a Django "model" is
    present, and the BASE class itself carries no model. A naive
    "cls.__init_subclass_with_meta__" dispatch would fire the base's own
    driver on the base and raise "AssertionError: ... received None" at
    import time (the S6b crasher).

    If this fails, defining a driver base class would immediately crash
    at import time (or silently misbehave) instead of only affecting
    subclasses.
    """
    from django_graphex.core.base import ObjectType
    ...
    class DriverBase(ObjectType):
        ...
        def __init_subclass_with_meta__(cls, model=None, **options):
            ...
            assert model is not None, f"{cls.__name__}: driver requires a model"
...
def test_intermediate_override_runs_only_on_subclasses() -> None:
    """Assert that a subclass overriding the driver runs the PARENT's driver, not its own.
    ...
    If this fails, an intermediate class's own driver override would fire
    on itself instead of only on classes further down the hierarchy.
    """
    from django_graphex.core.base import ObjectType
...
def test_meta_exposes_full_parity_attr_surface() -> None:
    """Assert that "_meta" exposes every parity-table attribute with no AttributeError.

    If this fails, a re-parented public type could hit a silent
    AttributeError when reading a graphene-parity Options attribute that
    the native Options object forgot to expose.
    """
    from django_graphex.core.base import ObjectType
...
def test_meta_defaults_match_graphene() -> None:
    """Assert that "_meta" defaults mirror graphene's Options defaults exactly.

    If this fails, a native Options default would diverge from the
    graphene baseline, changing observable behavior for re-parented types.
    """
    from django_graphex.core.base import ObjectType
...
def test_meta_is_mutable_plain_assignment() -> None:
    """Assert that "_meta" allows plain attribute assignment, with no freeze workaround.

    If this fails, later slices would still need the
    "object.__setattr__" freeze bypass instead of plain assignment.
    """
    from django_graphex.core.base import ObjectType
...
def test_meta_name_defaults_to_class_name() -> None:
    """Assert that "_meta.name" defaults to the class name when "Meta" omits "name".

    If this fails, a class without an explicit Meta.name would compile
    with a missing or incorrect GraphQL type name.
    """
    from django_graphex.core.base import ObjectType
...
def test_graphene_descriptors_collected_into_meta_fields() -> None:
    """Assert that native "field()" descriptors land in "_meta.fields".

    S8h: the 2.0 public field-declaration API is "field()" (graphene
    descriptors on a native type are no longer supported). A "field(...)"
    scalar / list / object descriptor must NOT be parsed as a Pydantic
    model field and must NOT crash class creation (the PydanticUserError
    "non-annotated attribute" blocker).

    If this fails, declared field descriptors would either crash class
    creation or fail to be collected for schema compilation.
    """
...
def test_graphene_descriptors_not_pydantic_fields() -> None:
    """Assert that native "field()" descriptors do not leak into Pydantic model_fields.

    If this fails, GraphQL-only field descriptors would be misinterpreted
    as Pydantic model fields, corrupting validation and serialization.
    """
    from graphql import GraphQLList, GraphQLString
...
def test_descriptors_survive_without_crash() -> None:
    """Assert that declaring native "field()" descriptors does not raise PydanticUserError.

    If this fails, any class declaring a field() descriptor in its body
    would crash at class-creation time instead of compiling cleanly.
    """
    from django_graphex.core import field
    from django_graphex.core.base import ObjectType
...
def test_container_constructor_round_trips_descriptor_payload() -> None:
    """Assert that "cls(**kwargs)" round-trips a payload keyed by descriptor field names.

    Mirrors graphene's mutation payload pattern "cls(**resp)" /
    "cls(**errors_dict)" (mutation.py:504,519; types.py:1750,1828).

    If this fails, constructing an instance from a keyword payload would
    lose or misassign one or more declared field values.
    """
    from graphql import GraphQLList, GraphQLString
...
def test_container_constructor_round_trips_driver_injected_field() -> None:
    """Assert that "cls(**resp)" round-trips a DYNAMIC output field injected by the driver.
    ...
    class-body descriptors), otherwise the dynamic output key is SILENTLY
    DROPPED by Pydantic and the mutation response's output object resolves
    to NULL on the wire (the S6c silent-null bug). This locks the
    "_meta.fields"-driven stash.

    If this fails, a mutation's dynamically injected output field would
    silently resolve to null on the wire instead of the real object.
    """
    ...
    from django_graphex.core import field
    from django_graphex.core.base import NativeObjectTypeOptions, ObjectType
...
def test_container_constructor_partial_payload() -> None:
    """Assert that "cls(**kwargs)" accepts a partial payload, defaulting missing keys safely.

    If this fails, omitting a declared field from the constructor payload
    would either crash or leak an unexpected stashed value into the
    instance, instead of falling back to the class-body descriptor.
    """
    from graphql import GraphQLList, GraphQLString
...
def test_concrete_driver_super_call_assigns_meta() -> None:
    """Assert that an S6b-shaped driver's super() call assigns "cls._meta" via the native terminal.
    ...
    the super() resolves to graphene's "BaseType" terminal (which
    freezes) or — after re-parent — to nothing; the native terminal is the
    load-bearing endpoint of the chain.

    If this fails, a concrete driver's super() call would not assign
    "_meta" at all, leaving the class without compiled Options state.
    """
    from django_graphex.core.base import NativeObjectTypeOptions, ObjectType
...
def test_terminal_defaults_name_to_class_name() -> None:
    """Assert that when the driver passes no "name", the terminal defaults to "cls.__name__".

    If this fails, a driver-built class without an explicit name option
    would compile with a missing or incorrect GraphQL type name.
    """
    from django_graphex.core.base import NativeObjectTypeOptions, ObjectType
...
def test_terminal_trims_docstring_into_description() -> None:
    """Assert that the terminal sets "_meta.description" to the trimmed class docstring.

    If this fails, a class's docstring would not become its GraphQL
    description, or would carry unwanted leading indentation.
    """
    from django_graphex.core.base import NativeObjectTypeOptions, ObjectType
...
def test_terminal_explicit_description_wins_over_docstring() -> None:
    """Assert that an explicit "description=" passed to the terminal wins over the docstring.

    If this fails, an explicitly supplied description would be overridden
    by the class docstring instead of taking precedence.
    """
    from django_graphex.core.base import NativeObjectTypeOptions, ObjectType
...
def test_terminal_keeps_meta_mutable_post_assignment() -> None:
    """Assert that the terminal does not freeze "_meta" — plain assignment works after it.

    The whole point of S6a is a mutable Options. graphene's "BaseType"
    calls "_meta.freeze()" before assigning; the native terminal must NOT,
    so later slices can plain-assign instead of the
    "object.__setattr__" workaround.

    If this fails, "_meta" would reject plain attribute assignment after
    the terminal runs, forcing callers back onto the freeze workaround.
    """
    from django_graphex.core.base import NativeObjectTypeOptions, ObjectType
...
def test_terminal_builds_meta_when_none() -> None:
    """Assert that a driver calling the terminal with "_meta=None" gets a fresh "_meta".

    The native terminal now collapses graphene's two-step
    "ObjectType.__init_subclass_with_meta__" (which builds an Options
    object when "_meta" is falsy) + "BaseType.__init_subclass_with_meta__"
    (name/desc/assign) into one endpoint. So a "_meta=None" call no
    longer no-ops — it builds a fresh mutable "NativeObjectTypeOptions",
    mirroring graphene's "ObjectType" parity (a plain "ObjectType" with no
    driver gets a real "_meta").

    If this fails, forwarding "_meta=None" to the terminal would leave
    the class without any Options object instead of building one.
    """
    from django_graphex.core.base import NativeObjectTypeOptions, ObjectType
...
def test_input_type_still_pydantic_model_metaclass() -> None:
    """Assert that an InputType subclass still has "type() is ModelMetaclass".

    If this fails, the S6a driver machinery would have regressed
    InputType's pre-existing Pydantic metaclass identity.
    """
...
def test_input_type_real_fields_still_work() -> None:
    """Assert that InputType real annotated fields still validate and round-trip.

    If this fails, the S6a driver machinery would have regressed normal
    Pydantic field validation, defaults, or dict-style access on InputType.
    """
    from django_graphex.core.base import InputType
...
def test_input_type_camel_alias_still_works() -> None:
    """Assert that InputType camelCase alias construction is unchanged.

    If this fails, the S6a driver machinery would have regressed
    "model_validate" construction from camelCase wire keys.
    """
    from django_graphex.core.base import InputType
...
def test_input_type_still_registers_and_has_input_meta() -> None:
    """Assert that InputType still auto-registers and keeps its graphql_input_type _meta.

    If this fails, the S6a driver machinery would have regressed the
    InputType registry auto-registration or its "_meta.graphql_input_type"
    attribute surface.
    """
