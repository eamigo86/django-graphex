...
def test_to_many_converters_return_native_marker_on_output() -> None:
    """Ships broken if the forward-M2M, reverse-FK, or reverse-M2M converters
    stop returning a "NativeRelationField" on the native OUTPUT path and
    instead return None or the dead-scalar sentinel.
    """
...
@pytest.mark.django_db
def test_nested_list_container_path_is_graphene_free() -> None:
    """Ships broken if the list-container path ("_nested_list_object_field"
    -> "get_or_create_list_object_type" -> "factory_type" ->
    "_build_list_object_field") fails to build for an already-registered node,
    which would mean the to-MANY OUTPUT CONTAINER is no longer built natively.
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_to_many_fields_are_list_containers_not_bare_lists() -> None:
    """Ships broken if a to-MANY field stops rendering as the related
    "<Model>ListType" results/totalCount CONTAINER (a native
    "GraphQLObjectType") and instead renders as a bare "[Node]" list.
    """
...
@pytest.mark.django_db
def test_list_container_keyed_in_registry() -> None:
    """Ships broken if the to-MANY "<Model>ListType" container stops being a
    native GraphQLObjectType keyed in the output registry, meaning nested
    references would re-create the container instead of reusing it.
    """
    from django_graphex.core.registry_compiler import compile_all_outputs
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_all_to_many_fields_present_in_compiled_output() -> None:
    """Ships broken if any to-MANY field (forward M2M, reverse FK, reverse
    M2M) is silently dropped from its compiled output type.
    """
    from django_graphex.core.registry_compiler import compile_all_outputs
...
@pytest.mark.django_db
def test_to_many_marker_present_in_meta_fields() -> None:
    """Ships broken if an AUTO-DERIVED to-MANY relation stops staying in
    "_meta.fields" as a "NativeRelationField" marker (presence/ordering) kept
    by "_yank_fields".
    """
    reg = Registry()
    _author, _tag, _comment, post_cls = _build_post_graph(reg)
    ...
    for name in ("tags", "co_authors", "comments"):
        assert name in fields, (
            f"to-MANY '{name}' must stay in _meta.fields (presence); got {list(fields)}"
        )
...
def test_seed_native_sdl_byte_identical_to_head_baseline() -> None:
    ...
    byte-identical to the HEAD (pre-S-rel-3) baseline — S-rel-3 must be
    SDL-NEUTRAL.
    """
...
@pytest.mark.django_db
def test_to_many_relation_shapes_unchanged_in_seed_sdl() -> None:
    """Ships broken if any to-MANY relation field shape changes in the seed
    SDL.
    """
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_generic_relation_list_returns_native_marker_in_s_rel_4() -> None:
    """Ships broken if the forward "GenericRelation" list converter stops
    returning a graphene-free "NativeRelationField" marker on the OUTPUT
    path.

    S-del-backend-11: the graphene backend is deleted, so the converter ALWAYS
    returns the native marker (the cross-backend "graphene.Dynamic" arm is
    gone).
    """
...
# --------------------------------------------------------------------------- #
def test_yank_fields_keeps_to_many_native_relation_field() -> None:
    """Ships broken if "_yank_fields" stops keeping a to-MANY
