...
root graphene-free with "from django_graphex.core import ObjectType, field" and that
...
# --------------------------------------------------------------------------- #
# Public API export — from django_graphex.core import ObjectType, field             #
# --------------------------------------------------------------------------- #
def test_public_objecttype_and_field_are_exported() -> None:
    """Assert that "ObjectType" and "field" are exported from "django_graphex.core".

    Both are importable from "django_graphex.core" and are the SAME objects as
    their canonical definitions; the package root no longer re-exports them.
    """
    import django_graphex
    from django_graphex.core import ObjectType, field
    from django_graphex.core.base import ObjectType as NativeObjectType
    from django_graphex.core.descriptors import field as native_field

    assert ObjectType is NativeObjectType
    assert field is native_field
    assert "ObjectType" not in django_graphex.__all__
    assert "field" not in django_graphex.__all__


# --------------------------------------------------------------------------- #
# field() class-body descriptor lands in _meta.fields on a ROOT class          #
# --------------------------------------------------------------------------- #
def test_field_on_native_root_lands_in_meta_fields() -> None:
...
# --------------------------------------------------------------------------- #
def test_native_root_mixed_fields_compile_and_execute_no_drop() -> None:
...
# --------------------------------------------------------------------------- #
def test_migrated_schema_query_compiles_native_with_custom_scalar_fields() -> None:
...
def test_migrated_schema_query_executes_custom_scalar_field() -> None:
