...
# ``PSCategory`` and ``OptNote.content_type`` / ``Track2GfkComment.target_ct``  #
# (FKs to the unregistered ``ContentType``) are dropped with a logged warning.  #
# --------------------------------------------------------------------------- #
...
class GfkS4Tag(models.Model):
    """Tag model carrying a flat GenericForeignKey, targeted by GfkS4Host's
    reverse GenericRelation."""
    ...
    class Meta:
        ...
        Registers the model under the "tests" app label.
        """
...
class GfkS4Host(models.Model):
    """Host model whose "tags" GenericRelation forces a reverse GenericRel
    onto "GfkS4Tag" (via related_query_name)."""
...
def _flat_gfk_field(model: type[models.Model]) -> GenericForeignKey:
    """Return the model's flat GenericForeignKey field."""
    return next(f for f in model._meta.get_fields() if isinstance(f, GenericForeignKey))


def _forward_generic_relation_field(model: type[models.Model]) -> GenericRelation:
    """Return the model's forward GenericRelation field."""
    return next(f for f in model._meta.get_fields() if isinstance(f, GenericRelation))


def _reverse_generic_rel_field(model: type[models.Model]) -> GenericRel:
    """Return the model's reverse GenericRel field."""
...
def test_flat_gfk_and_generic_relation_return_native_markers() -> None:
    """Ships broken if any of the flat GFK, forward GenericRelation, or
    reverse GenericRel OUTPUT converters stop returning a
    "NativeRelationField" and instead return None or the dead-scalar
    sentinel (a silent-drop trap).
    """
...
@pytest.mark.django_db
def test_full_gfk_object_type_build_does_not_import_graphene_for_relations() -> None:
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_gfk_union_output_renders_typed_union_in_schema() -> None:
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_reverse_generic_rel_arm_returns_native_marker() -> None:
    """Ships broken if the reverse "GenericRel" arm of
    "convert_many_rel_to_djangomodel" stops returning a "NativeRelationField"
    on the native OUTPUT path (the S-rel-3
    "not isinstance(field, GenericRel)" exclusion must stay flipped).
    """
    from django_graphex.converter import (
        _DEAD_SCALAR,
        convert_many_rel_to_djangomodel,
...
@pytest.mark.django_db
def test_reverse_generic_rel_not_rendered_in_native_output() -> None:
...
@pytest.mark.django_db
def test_forward_generic_relation_renders_native_list_container() -> None:
    """Ships broken if the forward "GenericRelation" stops rendering a
    "<Model>ListType" container in the native output (its Dynamic is dead —
    the container is built from "model._meta").
    """
    from django_graphex.core.registry_compiler import compile_all_outputs
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_graphene_marker_branch_retired_input_now_graphene_free() -> None:
    """Ships broken if any native build (OUTPUT or a CREATE-INPUT) still
    reaches "_yank_fields" with a graphene descriptor, or if
    "types._graphene_descriptor_markers" is still present.

    Part B retirement (S-input-5): the long-deferred Part B (#1609 /
    S-enum-2) is DONE — the INPUT path no longer emits graphene relation
    "Dynamic" / "ID" / choices "Enum" descriptors (they became
    "NativeRelationField" / dead-scalar / the shared native
    "GraphQLEnumType"). So:

    * the lazy "types._graphene_descriptor_markers" helper is DELETED;
    * a CREATE-INPUT build (formerly the blocker — it emitted graphene
      relation "Dynamic" descriptors that reached the branch via
      "_as.mounted") now reaches "_yank_fields" with ZERO graphene
      descriptors;
...
def test_seed_native_sdl_byte_identical_to_head_baseline() -> None:
    ...
    byte-identical to the HEAD (pre-S-rel-4) baseline — S-rel-4 must be
    SDL-NEUTRAL.
    """
    from tests.core._sdl_parity_seed import render_native_sdl
...
@pytest.mark.django_db
def test_flat_gfk_and_generic_relation_shapes_unchanged_in_seed_sdl() -> None:
    """Ships broken if the flat-GFK or GenericRelation field shapes change in
    the seed SDL.
    """
...
# --------------------------------------------------------------------------- #
def test_yank_fields_keeps_gfk_native_relation_field() -> None:
    """Ships broken if "_yank_fields" stops keeping a flat-GFK
    "NativeRelationField" AS-IS in its native-currency branch (it subclasses
    "NativeMountedField").
    """
...
# =========================================================================== #
@pytest.mark.django_db
def test_choices_define_object_type_does_not_import_graphene() -> None:
    """Ships broken if defining a "DjangoObjectType" with a choices field
    (class-def time) and building its native OUTPUT type calls
    "converter._g()" again (#1609 gap regression).
    """
...
@pytest.mark.django_db
def test_choices_output_converter_returns_dead_scalar() -> None:
    """Ships broken if "convert_django_field_with_choices" on OUTPUT
    (input_flag is None) for a choices field stops returning the dead-scalar
    sentinel or starts calling "_g()".
    """
    from django_graphex.converter import (
        _DEAD_SCALAR,
...
@pytest.mark.django_db
def test_choices_input_path_off_graphene() -> None:
    """Ships broken if the INPUT / mutation choices converter path stops
    returning the dead-scalar sentinel (S-input-5: the native input compiler
    renders the SHARED native "GraphQLEnumType" from "model._meta").
    """
...
@pytest.mark.django_db
def test_choices_field_omitted_from_meta_fields_on_output() -> None:
    """Ships broken if a choices field on OUTPUT stops being omitted from
    "_meta.fields" (option A: the dead-scalar sentinel, dropped by
    "construct_fields") — the native compiler still renders the enum from
    "model._meta", so this must stay SDL-neutral.
    """
    from tests.models import EnumCollisionItemA
...
@pytest.mark.django_db
def test_choices_output_build_clean_at_yank_fields() -> None:
    """Ships broken if a comprehensive native OUTPUT build reaches
    "_yank_fields" with a graphene descriptor (the choices Enum was the last
    graphene OUTPUT descriptor after S-rel-2/3/4; S-enum-2 retired it).

    S-input-5 retired the graphene-marker branch entirely, so there is no
    longer a "_graphene_descriptor_markers" to spy — the graphene-free
    assertion is the OUTPUT regression guard.
    """
    import django_graphex.types as types_mod
    from django_graphex.converter import _DEAD_SCALAR
    from django_graphex.core.descriptors import NativeField, NativeMountedField
...
@pytest.mark.django_db
def test_choices_seed_sdl_byte_identical_and_enum_renders() -> None:
    """Ships broken if the seed native SDL stops being byte-identical to HEAD
    (S-enum-2 must be SDL-NEUTRAL) or the choices field stops rendering as its
    enum (S-enum-1 output unchanged).
    """
    from tests.core._sdl_parity_seed import extract_type_block, render_native_sdl
    from tests.core.conftest import normalize_sdl
