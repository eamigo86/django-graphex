...
def _build_post_mutation_schema() -> tuple[
    DjangoGraphQLSchema, type[DjangoModelMutation]
]:
    """Build a native DjangoGraphQLSchema with a Post create+update mutation.

    Returns:
        pair: The assembled DjangoGraphQLSchema and the DjangoModelMutation
            class used to build its create/update fields.
    """
...
def test_native_mutation_field_output_is_payload_type_not_node() -> None:
    """Ships broken if the native mutation field's output type stops being
    the payload type (ok/errors + output field) and reverts to the bare
    model node type.
...
@pytest.mark.django_db
def test_native_mutation_field_arg_is_camelcase_wire_name() -> None:
    """Ships broken if the native mutation field's input argument stops using
    the camelCase wire name ("newPost") with "out_name" set to the snake
    Python kwarg ("new_post").
...
@pytest.mark.django_db
def test_native_create_mutation_executes_and_creates_row() -> None:
    """Ships broken if a native create mutation stops executing via
    graphql_sync, creating a real DB row, and returning the typed payload
    with the correct nested fields.
    """
...
@pytest.mark.django_db
def test_native_update_mutation_executes_and_updates_row() -> None:
    """Ships broken if a native update mutation stops executing via
    graphql_sync, mutating a real DB row, and returning the typed payload
