...
Run: .venv/bin/python -m pytest -q \
    tests/core/test_schema_registries.py
"""
...
def test_default_pair_returns_singleton() -> None:
    """Ships broken if "default_schema_registries()" stops returning the SAME
...
def test_default_pair_members_are_global_registries() -> None:
    """Ships broken if the default pair's two registries stop being the
    existing global instances.
    """
...
def test_default_pair_caches_are_module_level_dicts() -> None:
    """Ships broken if a cache field stops being the existing module-level
    dict (identity, not a copy).
...
def test_default_pair_write_through_is_visible_on_module_cache() -> None:
    """Ships broken if a write via the default pair's cache stops being
    observable on the module dict.
...
def test_non_default_pair_has_independent_caches() -> None:
    """Ships broken if a fresh "SchemaRegistries" stops being independent of
    the global module dicts.
...
def test_resolve_registries_defaults_to_global_pair() -> None:
    """Ships broken if a compiler's "_resolve_registries(None)" stops
    yielding the default pair.
    ...
    """
    from django_graphex.core import polymorphic_compiler, schema_compiler
    from django_graphex.core.base import default_schema_registries
