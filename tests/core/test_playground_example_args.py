...
"class Arguments" USED to be declared with the transitional "graphene.Argument"
...
API (a graphql-core "GraphQLArgument" / lazy thunk) inside "class Arguments".
...
def test_playground_create_category_has_data_arg() -> None:
    """Ships broken if "createCategory" stops exposing its "data:
    CategoryInput!" arg in the generated SDL (the arg gets silently dropped).
    """
...
def test_playground_upload_document_has_both_args() -> None:
    """Ships broken if "uploadDocument" stops exposing both "name: String!"
    and "file: Base64FileInput!" in the generated SDL.
    """
    lines = _build_playground_mutation_sdl()
