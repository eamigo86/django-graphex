...
def test_django_input_object_type_graphql_input_type_create() -> None:
    """Ships broken if a "create" DjangoInputObjectType stops exposing a
    compiled "graphql_input_type" carrying the GdxInputSpec extension.
    """
...
@pytest.mark.django_db
def test_django_input_object_type_graphql_input_type_update_all_nullable() -> None:
    """Ships broken if an "update" DjangoInputObjectType (partial model) stops
    rendering every field as nullable.
    """
...
@pytest.mark.django_db
def test_model_free_meta_container_absent() -> None:
    """Ships broken if the model-free "_GdxInputMeta" starts exposing a
    "container" attribute — the native model-free metaclass has no container
    slot.
    ...
    """
    from django_graphex.core.base import _GdxInputMeta, _GdxInputOptions
...
@pytest.mark.django_db
def test_django_input_object_type_meta_container_absent_after_s6c() -> None:
    """Ships broken if "DjangoInputObjectType._meta.container" stops being
    absent after S6c.
...
@pytest.mark.django_db
def test_assert_input_flag_guard() -> None:
    """Ships broken if "compile_input_type" stops rejecting a None model — the
    "assert input_flag is not None" guard.
...
def test_gdx_input_meta_graphql_input_type_exposed() -> None:
    """Ships broken if "_GdxInputMeta" stops exposing "graphql_input_type"
    after compilation.
    """
    from graphql import GraphQLInputObjectType
...
def test_gdx_input_meta_unknown_attr_raises() -> None:
    """Ships broken if "_GdxInputMeta" stops raising AttributeError for
    unknown attributes.
    """
    from django_graphex.core.base import _GdxInputMeta, _GdxInputOptions
...
def test_type_of_model_free_input_is_model_metaclass() -> None:
    """Ships broken if "type(InputType subclass)" stops being pydantic's
    ModelMetaclass — the Phase-2 deliverable for model-free types.
...
@pytest.mark.django_db
def test_build_model_schema_returns_pydantic_model() -> None:
    """Ships broken if "build_model_schema" stops returning an intermediate
    ...
    "_meta.graphql_input_type" being a real GraphQLInputObjectType (see
    "test_django_input_object_type_meta_graphql_input_type_is_graphql_type").
    """
    from pydantic._internal._model_construction import ModelMetaclass
...
@pytest.mark.django_db
def test_model_coupled_metaclass_is_modelmetaclass_after_s6c() -> None:
    """Ships broken if "type(DjangoInputObjectType subclass)" stops being
    pydantic's ModelMetaclass after S6c.
    ...
    is pydantic "ModelMetaclass" (no custom metaclass — the graphene-free
    driver runs via "__init_subclass__"). This is the systemic
    metaclass-identity constraint #1452: every public "Django*" type keeps
    "type(X) is ModelMetaclass".
    ...
    assert isinstance(
        _S6cModelCoupledInput._meta.graphql_input_type, GraphQLInputObjectType
    ), (
...
@pytest.mark.django_db
def test_django_input_object_type_does_not_pollute_input_registry() -> None:
    """Ships broken if a DjangoInputObjectType subclass starts registering in
    "_gdx_input_registry".
    ...
    """
    from django_graphex.core.base import _gdx_input_registry
    from django_graphex.types import DjangoInputObjectType
    from tests.models import Category
...
def test_gdx_input_options_graphql_input_type_default_none() -> None:
    """Ships broken if "_GdxInputOptions.graphql_input_type" stops defaulting
    to None before compilation.
    """
    from django_graphex.core.base import _GdxInputOptions

    opts = _GdxInputOptions()
    assert opts.graphql_input_type is None


def test_input_type_meta_populated_by_compile_all_inputs() -> None:
    """Ships broken if "InputType._meta.graphql_input_type" stops being None
    before "compile_all_inputs" runs.
    """
    from django_graphex.core.base import InputType
...
@pytest.mark.django_db
def test_django_input_object_type_meta_graphql_input_type_is_graphql_type() -> None:
    """Ships broken if a DjangoInputObjectType subclass stops setting
    "_meta.graphql_input_type" to a real GraphQLInputObjectType at class
    creation.

    This is the core integration gate for WU-B: the production
    "__init_subclass_with_meta__" must call "compile_input_type"; the existing
    graphene path must remain intact.
    """
    from graphql import GraphQLInputObjectType
...
@pytest.mark.django_db
def test_django_input_object_type_native_no_container_created() -> None:
    """Ships broken if the native resolver path stops reading
    "_meta.graphql_input_type" directly (with no container in between).

    Graphene still has a container for its own schema build, but that is a
    graphene-internal detail we don't test here.
    """
    from graphql import GraphQLInputObjectType
...
@pytest.mark.django_db
def test_django_input_object_type_update_meta_all_fields_nullable() -> None:
    """Ships broken if an update input (partial=True) stops rendering every
    field as nullable under native.
    """
...
@pytest.mark.django_db
def test_mutation_arguments_use_graphql_argument_under_native() -> None:
    """Ships broken if "DjangoModelType._meta.arguments['create']" stops
    holding a dict with a GraphQLArgument (instead of a graphene Argument).

    This is the "call site" integration test: when the 6
    "factory_type("input", ...)" call sites are properly wired,
    "global_arguments[op][input_field_name]" must be a graphql-core
    GraphQLArgument, not a graphene Argument.
    """
    ...
    assert isinstance(input_arg, GraphQLArgument), (
        f"mutation input arg must be GraphQLArgument, got {type(input_arg)}"
    )
