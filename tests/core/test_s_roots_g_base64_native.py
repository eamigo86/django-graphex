...
def test_base64_file_input_is_native_input_type() -> None:
    """Assert that "Base64FileInput" is a native InputType (Pydantic), not graphene.

    Ships broken if "Base64FileInput" stops subclassing the native
    "InputType" base and the Pydantic "BaseModel" it wraps.
    """
...
def test_base64_file_input_fields_declared() -> None:
    """Assert that the Pydantic model declares filename, data, content_type fields.

    Ships broken if any of the three input fields silently vanishes from
    "Base64FileInput.model_fields".
    """
...
def test_validated_instance_to_uploaded_file() -> None:
    """Assert that a validated instance's ".to_uploaded_file()" decodes for real.

    Ships broken if calling ".to_uploaded_file()" on a validated
    "Base64FileInput" stops producing a Django "SimpleUploadedFile" whose
    bytes match the decoded base64 payload.
    """
...
def test_validated_instance_default_content_type() -> None:
    """Assert that omitting contentType defaults content_type to octet-stream.

    Ships broken if an input validated without a "contentType" key stops
    defaulting the produced upload's content type to
    "application/octet-stream".
    """
    value = Base64FileInput.model_validate({"filename": "blob.bin", "data": _HELLO_B64})
...
def test_validated_instance_over_limit_raises() -> None:
    """Assert that "to_uploaded_file(max_size=N)" enforces a per-file size cap.

    Ships broken if decoding a payload larger than "max_size" stops raising
    a "GraphQLError" naming the size violation.
    """
    large_data = base64.b64encode(b"x" * 200).decode()
    value = Base64FileInput.model_validate({"filename": "big.bin", "data": large_data})
...
def test_base64_file_input_compiles_to_graphql_input_type() -> None:
    """Assert that "Base64FileInput" compiles to a "GraphQLInputObjectType".

    Ships broken if the compiled input type stops carrying camelCase wire
    keys ("contentType") mapped back to the snake_case attribute via
    "out_name".
    """
...
def test_decode_base64_file_helper_unchanged() -> None:
    """Assert that "decode_base64_file(dict)" still decodes to a SimpleUploadedFile.

    Ships broken if the standalone dict-based decode helper stops producing
    a Django "SimpleUploadedFile" with the right name/content-type/bytes.
    """
