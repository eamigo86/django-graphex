...
def test_native_list_exposes_of_type() -> None:
    """Assert that "NativeList(inner)" exposes ".of_type" (the wrapper read-contract).
...
def test_field_with_native_list_errortype_compiles_to_list_of_errortype() -> None:
    """Assert that "field(NativeList(ErrorType))" compiles to "[ErrorType]" (lazy).
    ...
    from django_graphex.core.descriptors import NativeList, field
...
def test_ok_errors_descriptors_are_native_fields(import_path: str) -> None:
    """Assert that "ok"/"errors" class-body descriptors are native "NativeField" instances.
    ...
    Args:
        import_path: The dotted module path ("django_graphex.types" or
            "django_graphex.mutation") to import and inspect.
    """
    import importlib
    ...
    assert isinstance(errors_desc, NativeField), (
        f"{cls_name}.errors is not a NativeField"
    )


@pytest.mark.django_db
def test_modeltype_ok_errors_in_meta_fields() -> None:
    """Assert that "ok"/"errors" land in "_meta.fields" — the S6c stash mechanism.
...
def test_mutation_ok_errors_in_meta_fields() -> None:
    """Assert that "ok"/"errors" land in "DjangoModelMutation._meta.fields" (S6c stash).

    The parametrized twin of "test_modeltype_ok_errors_in_meta_fields" for the
    mutation-payload side of the same S6c stash mechanism.
    """
...
def test_wire_mutation_returns_non_null_ok_and_queryable_errors() -> None:
    ...
    would silently null "ok", and the assertion 'data["ok"] is True' would FAIL.
    """
