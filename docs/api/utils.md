...
This section provides detailed API documentation for utility functions in `django-graphex`.
...
from django_graphex.utils import get_reverse_fields
...
from django_graphex.utils import get_related_model
...
**Example:**
```python
from django_graphex.utils import get_obj
...
**Example:**
```python
from django_graphex.utils import create_obj
...
**Parameters:**
- `klass` (`Model`, `Manager`, or `QuerySet`): Source to query
- `*args`: When given, the first value selects the database alias
- `**kwargs`: Lookup parameters for the `get()` query

**Returns:** Model instance or `None`

**Example:**
```python
from django_graphex.utils import get_Object_or_None
...
**Example:**
```python
from django_graphex.utils import to_kebab_case
...
**Example:**
```python
from django_graphex.utils import clean_dict
...
from django_graphex.utils import get_type
...
**Example:**
```python
from django_graphex.utils import is_required
...
**Example:**
```python
from django_graphex.utils import _get_queryset
...
`queryset_factory` inspects the GraphQL selection in `info` and automatically
applies nested `select_related` / `prefetch_related` (and, when
`OPTIMIZE_ONLY_FIELDS` is enabled, a conservative `.only()` projection). You do
not pass `select_related`/`prefetch_related` yourself — extra `**kwargs` are the
resolver arguments used to seed relation joins for filtering.

**Example:**
```python
from django_graphex.utils import queryset_factory

def resolve_users(self, info, **kwargs):
    # Builds a queryset optimized for the requested selection set.
    return queryset_factory(User.objects, None, info, **kwargs)
```
...
**Parameters:**
- `field` (`FieldNode`): GraphQL AST field node whose name is looked up
- `fields_dict` (`dict`): Mapping of field names to fields
...
**Example:**
```python
from django_graphex.utils import get_related_fields
...
```python
from django_graphex.utils import get_fields, is_required
...
```python
from django_graphex.utils import create_obj, get_obj
...
```python
from django_graphex.utils import get_model_fields, is_required
...
```python
from django_graphex.utils import queryset_factory, get_fields
...
This comprehensive API reference covers all utility functions in `django-graphex`, providing developers with powerful tools for Django-GraphQL integration and optimization.
